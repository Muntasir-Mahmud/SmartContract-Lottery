...
def get_contract(contract_name):
    ...
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENT:
        if len(contract_type) <=0:
            deploy_mock()
        contract = contract_type[-1] # Grab the latest contarct eg. MockV3Aggregator[-1]
    else:
...
def fund_with_link(contract_address, account=None, link_token=None, amount=100000000000000000): # 10^17 or 0.1 LINK
